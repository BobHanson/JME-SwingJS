...
/**
 * TODO: handle the text width with automatic centering drawCenteredString(....
...
public class PreciseGraphicsAWT {
	...
	public class ExtendedGraphics2D {
		...
		// BH 2023.01.18 inner class -- no need to duplicate references to original
		// graphics object.
	...
	public void initPrecisionScale(double absoluteScale) {
		...
		// relative to the previous scaling.
...
//		}
	...
	public void setColor(Color color) {
		// if override color is used, than only one color is actually used for the
		// foreground
	...
	/**
	 * Reimplementation needed because the original AWT uses a linewidth equal to 1,
	 * which is too small
	 * 
	 * @param x
	 * @param y
	...
	public void fill3DRect(double x, double y, double width, double height, boolean raised) {
		...
		this.baseGraphics.fillRect(xInt + widthInt, yInt, border, heightInt + border); // BB added + border, looks
																						// better
	...
	/**
	 * Used for text with a colored background to make the text more readable. Works
	 * only in HiDPI screen or large zoom (200%)
	 * 
	 * @param str
	 * @param x
	 * @param y
	 ...
	 *                    superscript. [[2,2]], 13C -> [[0.2]]
	 */
	public void drawStringWithStrokeAndBaselineShifts(String str, double x, double y, Color strokeColor,
			double strokeWidth, int subscripts[][], int superscript[][]) {
		extendedBaseGraphics.drawStringWithStrokeAndBaselineShifts(str, r(x), r(y), strokeColor, r(strokeWidth),
